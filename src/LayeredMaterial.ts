  ...
  private computeTopologySignature(layers: LayerConfig[]): string {
    return layers
      .map((layer) => JSON.stringify({
        ...
        triplanarSharpness: layer.triplanar?.enable ? (layer.triplanar.sharpness ?? 4.0) : 4.0,
        triplanarCutoff: layer.triplanar?.enable ? (layer.triplanar.cutoff ?? 0.0) : 0.0,
        textureBombing: !!layer.textureBombing?.enable,
        textureBombingMode: layer.textureBombing?.enable ? (layer.textureBombing.mode ?? 'voronoi') : 'disabled',
        ...
        textureBombingNormalCorrection: layer.textureBombing?.enable ? (layer.textureBombing.normalCorrection ?? false) : false,
        textureBombingScaleJitter: layer.textureBombing?.enable ? (layer.textureBombing.scaleJitter ?? 0) : 0,
